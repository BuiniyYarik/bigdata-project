...
if ! beeline -u jdbc:hive2://hadoop-03.uni.innopolis.ru:10001 -n team4 -p "$password" \
  -f $base_path/sql/db.hql \
  --silent=true --outputformat=table \
  > $base_path/output/temp_hive_results.txt 2> /dev/null; then
    echo "Failed to create database and temporary table in Hive."
    exit 1
fi

echo "Finished temp Hive table creation."
...
if ! beeline -u jdbc:hive2://hadoop-03.uni.innopolis.ru:10001 -n team4 -p "$password"  \
  ...
  --hiveconf hive.tez.container.size=3072 \
  --hiveconf hive.tez.java.opts=-Xmx2450m \
  --hiveconf tez.am.resource.memory.mb=1024 \
  --hiveconf mapreduce.job.reduces=3 \
  --hiveconf hive.tez.auto.reducer.parallelism=false \
  -f $base_path/sql/create_table.hql > /dev/null 2>&1; then
...
if ! beeline -u jdbc:hive2://hadoop-03.uni.innopolis.ru:10001 -n team4 -p "$password" \
  ...
  --hiveconf mapreduce.job.reduces=3 \
  --hiveconf hive.tez.auto.reducer.parallelism=false \
  -f $base_path/sql/import_from_temp.hql > /dev/null 2>&1; then
...
for i in "${queries[@]}"; do
    ...
    if ! beeline -u jdbc:hive2://hadoop-03.uni.innopolis.ru:10001 -n team4 -p "$password"  \
      ...
      --silent=true --outputformat=dsv --delimiterForDSV=, \
      -f $base_path/sql/q"$i".hql > $base_path/output/q"$i".csv 2>/dev/null; then
        echo "Failed to execute query $i."
        exit 1
    fi
done

printf "EDA and insights creation completed successfully!\n"
printf "\n"
