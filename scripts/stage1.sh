...
hdfs dfs -rm -r -f -skipTrash /user/team4/project/warehouse > /dev/null 2>&1
...
sqoop import-all-tables \
  --connect jdbc:postgresql://hadoop-04.uni.innopolis.ru/team4_projectdb \
  --username team4 \
  --password "$password" \
  --compression-codec snappy \
  --compress \
  --as-avrodatafile \
  --warehouse-dir /user/team4/project/warehouse \
  --m 1 2> /dev/null
